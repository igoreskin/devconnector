...
import { Link, withRouter } from 'react-router-dom'; // withRouter - to be able to redirect with history object in the action creator 
      ...
      <form class="form" onSubmit={e => { e.preventDefault(); addEducation(formData, history) }}>
        ...
        <div class="form-group">
          <p><input type="checkbox" name="current" checked={current} value={current} onChange={e => {
            setFormData({ ...formData, current: !current }); // toggling when checking and unchecking the checkbox 
            toggleDisabled(!toDateDisabled); // toggling toDateDisabled property enabling and disabling the ToDate field 
          }} /> {' '}Current</p>
        </div>
        <div class="form-group">
          <h4>To Date</h4>
